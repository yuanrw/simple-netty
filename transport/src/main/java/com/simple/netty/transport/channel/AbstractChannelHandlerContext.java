...
import static com.simple.netty.transport.channel.ChannelHandlerMask.*;
...
public abstract class AbstractChannelHandlerContext implements ChannelHandlerContext {
    ...
    /**
     * 即将调用 {@link ChannelHandler#handlerAdded(ChannelHandlerContext)}
     */
    ...
    static void invokeChannelRead(final AbstractChannelHandlerContext next, Object msg) {
        final ByteBuf m = ObjectUtil.checkNotNull((ByteBuf) msg, "msg");
        EventExecutor executor = next.executor();
        if (executor.inEventLoop()) {
    ...
    private void invokeChannelRead(ByteBuf msg) {
        if (invokeHandler()) {
            try {
    ...
    public ChannelHandlerContext read() {
        ...
        if (executor.inEventLoop()) {
            ...
            executor.execute(next::invokeRead);
        }
    ...
    public ChannelHandlerContext flush() {
        ...
        if (executor.inEventLoop()) {
            ...
            Runnable flush = next::invokeFlush;
            safeExecute(executor, flush, channel().voidPromise(), null);
        }
    ...
    private void write(Object msg, boolean flush, ChannelPromise promise) {
        ...
        if (executor.inEventLoop()) {
            ...
            final Runnable task = () -> {
                next.pipeline.decrementPendingOutboundBytes(size);
                if (flush) {
                    next.invokeWriteAndFlush(msg, promise);
                } else {
                    next.invokeWrite(msg, promise);
            ...
            if (!safeExecute(executor, task, promise, msg)) {
                //没有提交成功，需要减少pendingOutboundBytes
                next.pipeline.decrementPendingOutboundBytes(size);
            }
        }
    }
